...
async function answerall(req, res) {
    try {
        ...
        const query2 ="SELECT u.username, q.answer FROM newuser2 u INNER JOIN answers q ON u.userid = q.userid where 	questionid=?";
...
async function answer(req,res){
    ...
    const query=" insert in to answer (questionid,userid,answer) values (?,?,?)"
...
async function allanswers(req, res) {
    try {
      ...
      const query="SELECT u.username, q.answer FROM newuser2 u INNER JOIN answers q ON u.userid = q.userid where 	questionid=? ORDER BY q.answerid DESC";
